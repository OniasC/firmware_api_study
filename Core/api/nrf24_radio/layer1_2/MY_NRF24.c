...
#include "../../nrf24_radio/layer1_2/MY_NRF24.h"
...
uint8_t NRF24_readRegister_v2(nrf24_t * const radio, uint8_t reg, uint8_t * ret)
{
	...
	//Transmit register address
	spiBuf[0] = reg&0x1F;
	HAL_SPI_TransmitReceive(radio->chip_spi, &spiBuf[0], ret, 1, 100);/*TODO: this is not right...*/
	//* ret = spiBuf[0];
	//Receive data
	radio->spi_status = HAL_SPI_Receive(radio->chip_spi, &spiBuf[1], 1, 100);
	retData = spiBuf[1];
...
{
	NRF24_writeRegister(radio, NRF24_CMD_FLUSH_TX, 0xFF);
}
//10. Flush Rx buffer
void NRF24_flush_rx(nrf24_t * const radio)
{
	NRF24_writeRegister(radio, NRF24_CMD_FLUSH_RX, 0xFF);
}
//11. Get status register value
uint8_t NRF24_get_status(nrf24_t * const radio)
{
	uint8_t statReg;
	statReg = NRF24_readRegister(radio, NRF24_REG_STATUS);
	return statReg;
}
...
void NRF24_begin(nrf24_t * const radio)
{
	...
	NRF24_writeRegister(radio, NRF24_REG_RX_PW_P2, 0);
	NRF24_writeRegister(radio, NRF24_REG_RX_PW_P3, 0);
	NRF24_writeRegister(radio, NRF24_REG_RX_PW_P4, 0);
	NRF24_writeRegister(radio, NRF24_REG_RX_PW_P5, 0);

	NRF24_ACTIVATE_cmd(radio);
	NRF24_writeRegister(radio, 0x1c, 0);
	NRF24_writeRegister(radio, 0x1d, 0);
...
/*
 * radio.begin() is different than the reference code. check again why...
 *
 *
 * */
...
bool NRF24_write(nrf24_t * const radio,  const void* buf, uint8_t len )
{
  ...
  uint32_t sent_at = HAL_GetTick();
	...
	do
  {
    NRF24_readRegisterN(radio, NRF24_REG_OBSERVE_TX,&observe_tx,1);
		//Get status register
...
{
	NRF24_readPayload(radio,  buf, len );
	uint8_t rxStatus = NRF24_readRegister(radio, NRF24_REG_FIFO_STATUS) & _BV(BIT_RX_EMPTY);
	NRF24_flush_rx(radio);
...
{
	NRF24_writeRegister(radio, NRF24_REG_SETUP_RETR,(delay&0xf)<<BIT_ARD | (count&0xf)<<BIT_ARC);
}

//21. Set RF channel frequency
void NRF24_setChannel(nrf24_t * const radio, uint8_t channel)
{
  NRF24_writeRegister(radio, NRF24_REG_RF_CH, MIN(channel, NRF24_MAX_CHANNEL));
...
{
	return NRF24_readRegister(radio, NRF24_CMD_R_RX_PL_WID);
}
//25. Enable payload on Ackknowledge packet
void NRF24_enableAckPayload(nrf24_t * const radio)
{
	//Need to enable dynamic payload and Ack payload together
	 NRF24_writeRegister(radio, NRF24_REG_FEATURE, NRF24_readRegister(radio, NRF24_REG_FEATURE) | _BV(BIT_EN_ACK_PAY) | _BV(BIT_EN_DPL) );
	if(!NRF24_readRegister(radio, NRF24_REG_FEATURE))
	{
		NRF24_ACTIVATE_cmd(radio);
		NRF24_writeRegister(radio, NRF24_REG_FEATURE, NRF24_readRegister(radio, NRF24_REG_FEATURE) | _BV(BIT_EN_ACK_PAY) | _BV(BIT_EN_DPL) );
	}
	// Enable dynamic payload on pipes 0 & 1
	NRF24_writeRegister(radio, NRF24_REG_DYNPD, NRF24_readRegister(radio, NRF24_REG_DYNPD) | _BV(BIT_DPL_P1) | _BV(BIT_DPL_P0));
    radio->dynamic_payloads_enabled = true;
    radio->ack_payloads_enabled = true;
    /*different than nrf24 library... check later on if makes a difference*/
}
...
void NRF24_disableAckPayload(nrf24_t * const radio)
{
    ...
    if (radio->ack_payloads_enabled){
    	NRF24_writeRegister(radio, NRF24_REG_FEATURE, NRF24_readRegister(radio, NRF24_REG_FEATURE) | ~_BV(BIT_EN_ACK_PAY));
...
void NRF24_enableDynamicPayloads(nrf24_t * const radio)
{
	...
	NRF24_writeRegister(radio, NRF24_REG_FEATURE, NRF24_readRegister(radio, NRF24_REG_FEATURE) |  _BV(BIT_EN_DPL) );
	if(!NRF24_readRegister(radio, NRF24_REG_FEATURE))
	{
		NRF24_ACTIVATE_cmd(radio);
		NRF24_writeRegister(radio, NRF24_REG_FEATURE, NRF24_readRegister(radio, NRF24_REG_FEATURE) |  _BV(BIT_EN_DPL) );
	...
	NRF24_writeRegister(radio, NRF24_REG_DYNPD, NRF24_readRegister(radio, NRF24_REG_DYNPD) | _BV(BIT_DPL_P5) | _BV(BIT_DPL_P4) | _BV(BIT_DPL_P3) | _BV(BIT_DPL_P2) | _BV(BIT_DPL_P1) | _BV(BIT_DPL_P0));
  radio->dynamic_payloads_enabled = true;
}
...
void NRF24_disableDynamicPayloads(nrf24_t * const radio)
{
	...
	NRF24_writeRegister(radio, NRF24_REG_DYNPD, 0);
...
void NRF24_setAutoAck(nrf24_t * const radio, bool enable)
{
	...
	{
		NRF24_writeRegister(radio, NRF24_REG_EN_AA, 0x00);
		if(radio->ack_payloads_enabled)
		{
			NRF24_disableAckPayload(radio);
		}
	}
}
...
void NRF24_setAutoAckPipe(nrf24_t * const radio,  uint8_t pipe, bool enable )
{
	if ( pipe <= 6 )
  {
    uint8_t en_aa = NRF24_readRegister(radio,  NRF24_REG_EN_AA) ;
    if( enable )
    {
      en_aa |= _BV(pipe);
    }
    ...
    {
      en_aa &= ~_BV(pipe);
      if(radio->ack_payloads_enabled && !pipe)
      {
    	  NRF24_disableAckPayload(radio);
      }
    }
    NRF24_writeRegister(radio,  NRF24_REG_EN_AA, en_aa ) ;
  }
}
//30. Set transmit power level
void NRF24_setPALevel(nrf24_t * const radio,  nrf24_pa_dbm_e level )
{
	uint8_t setup = NRF24_readRegister(radio, NRF24_REG_RF_SETUP) ;
	...
	NRF24_writeRegister(radio,  NRF24_REG_RF_SETUP, setup );
...
nrf24_pa_dbm_e NRF24_getPALevel( nrf24_t * const radio )
{
  ...
  uint8_t power = NRF24_readRegister(radio, NRF24_REG_RF_SETUP) & (_BV(RF_PWR_LOW) | _BV(RF_PWR_HIGH));
...
bool NRF24_setDataRate(nrf24_t * const radio, nrf24_datarate_e speed)
{
  ...
  // Verify our result
  if ( NRF24_readRegister(radio, NRF24_REG_RF_SETUP) == setup )
  {
...
nrf24_datarate_e NRF24_getDataRate( nrf24_t * const radio )
{
  ...
  uint8_t dr = NRF24_readRegister(radio, NRF24_REG_RF_SETUP) & (_BV(RF_DR_LOW) | _BV(RF_DR_HIGH));
...
{
	uint8_t config = NRF24_readRegister(radio, NRF24_REG_CONFIG) & ~( _BV(BIT_CRCO) | _BV(BIT_EN_CRC)) ;
  ...
  NRF24_writeRegister(radio,  NRF24_REG_CONFIG, config );
}
//35. Get CRC length
...
{
	uint8_t disable = NRF24_readRegister(radio, NRF24_REG_CONFIG) & ~_BV(BIT_EN_CRC) ;
  NRF24_writeRegister(radio,  NRF24_REG_CONFIG, disable ) ;
}
//37. power up
void NRF24_powerUp(nrf24_t * const radio)
{
	NRF24_writeRegister(radio, NRF24_REG_CONFIG, NRF24_readRegister(radio, NRF24_REG_CONFIG) | _BV(BIT_PWR_UP));
}
//38. power down
...
bool NRF24_availablePipe(nrf24_t * const radio, uint8_t* pipe_num)
{
  ...
  if (result)
  {
    ...
    NRF24_writeRegister(radio, NRF24_REG_STATUS,_BV(BIT_RX_DR) );

    // Handle ack payload receipt
    if ( status & _BV(BIT_TX_DS) )
    {
      NRF24_writeRegister(radio, NRF24_REG_STATUS,_BV(BIT_TX_DS));
    }
  }
  return result;
}
...
void NRF24_startWrite(nrf24_t * const radio,  const void* buf, uint8_t len )
{
  ...
  // Send the payload
  NRF24_writePayload(radio,  buf, len );
...
void NRF24_writeAckPayload(nrf24_t * const radio, uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  NRF24_csn(radio, 0);
	NRF24_writeRegisterN(radio, NRF24_CMD_W_ACK_PAYLOAD | ( pipe & 0x7 ) , current, data_len);
  NRF24_csn(radio, 1);
}
...
void NRF24_whatHappened(nrf24_t * const radio, bool *tx_ok, bool *tx_fail, bool *rx_ready)
{
	uint8_t status = NRF24_get_status(radio);
	*tx_ok = 0;
	NRF24_writeRegister(radio, NRF24_REG_STATUS,_BV(BIT_RX_DR) | _BV(BIT_TX_DS) | _BV(BIT_MAX_RT) );
...
bool NRF24_txStandBy(nrf24_t * const radio, uint32_t timeout, bool startTx)
{
    ...
    uint8_t ret;
    uint8_t tx_status = NRF24_readRegister_v2(radio, NRF24_REG_FIFO_STATUS, &ret);
    //uint8_t tx_status = NRF24_readRegister(radio, NRF24_REG_FIFO_STATUS);
    while (!(tx_status & _BV(BIT_TX_EMPTY)))
    {
        if (ret & _BV(MASK_MAX_RT))
        {
        	/*TODO: fix this!!
        	 * tx_status[4th bit]: tx fifo empty. (1: tx fifo empty; 0: data in tx fifo)
        	 *    //this is the status bit.
        	 *    arduino original code:
        	 *    status = _SPI.transfer(R_REGISTER | NRF24_REG_FIFO_STATUS);
        	 *
        	 * MASK_MAX_TX: maximum number of TX retransmits interrupt. Write 1 to clear bit.
        	 * if MAX_RT is asserted, but be cleared to enable further comms.
        	 * */
        	NRF24_writeRegister(radio, NRF24_REG_STATUS, _BV(MASK_MAX_RT));
...
void printRadioSettings(nrf24_t * const radio)
{
	...
	reg8Val = NRF24_readRegister(radio, 0x00);
	if(reg8Val & (1 << 3))
	{
	...
	API_DEBUG_MESSAGE(uartTxBuf);
	//b) AutoAck on pipes
	reg8Val = NRF24_readRegister(radio, 0x01);
	sprintf(uartTxBuf, "ENAA:\r\n		P0:	%d\r\n		P1:	%d\r\n		P2:	%d\r\n		P3:	%d\r\n		P4:	%d\r\n		P5:	%d\r\n",
	_BOOL(reg8Val&(1<<0)), _BOOL(reg8Val&(1<<1)), _BOOL(reg8Val&(1<<2)), _BOOL(reg8Val&(1<<3)), _BOOL(reg8Val&(1<<4)), _BOOL(reg8Val&(1<<5)));
	API_DEBUG_MESSAGE(uartTxBuf);
	//c) Enabled Rx addresses
	reg8Val = NRF24_readRegister(radio, 0x02);
	sprintf(uartTxBuf, "EN_RXADDR:\r\n		P0:	%d\r\n		P1:	%d\r\n		P2:	%d\r\n		P3:	%d\r\n		P4:	%d\r\n		P5:	%d\r\n",
	_BOOL(reg8Val&(1<<0)), _BOOL(reg8Val&(1<<1)), _BOOL(reg8Val&(1<<2)), _BOOL(reg8Val&(1<<3)), _BOOL(reg8Val&(1<<4)), _BOOL(reg8Val&(1<<5)));
	API_DEBUG_MESSAGE(uartTxBuf);
	//d) Address width
	reg8Val = NRF24_readRegister(radio, 0x03)&0x03;
	reg8Val +=2;
	sprintf(uartTxBuf, "SETUP_AW:\r\n		%d bytes \r\n", reg8Val);
	API_DEBUG_MESSAGE(uartTxBuf);
	//e) RF channel
	reg8Val = NRF24_readRegister(radio, 0x05);
	sprintf(uartTxBuf, "RF_CH:\r\n		%d CH \r\n", reg8Val&0x7F);
	API_DEBUG_MESSAGE(uartTxBuf);
	//f) Data rate & RF_PWR
	reg8Val = NRF24_readRegister(radio, 0x06);
	...
	uint8_t pipeAddrs[6];
	NRF24_readRegisterN(radio, 0x0A, pipeAddrs, 5);
	sprintf(uartTxBuf, "RX_Pipe0 Addrs:\r\n		%02X,%02X,%02X,%02X,%02X  \r\n", pipeAddrs[4], pipeAddrs[3], pipeAddrs[2],pipeAddrs[1],pipeAddrs[0]);
	API_DEBUG_MESSAGE(uartTxBuf);

	NRF24_readRegisterN(radio, 0x0A+1, pipeAddrs, 5);
	sprintf(uartTxBuf, "RX_Pipe1 Addrs:\r\n		%02X,%02X,%02X,%02X,%02X  \r\n", pipeAddrs[4], pipeAddrs[3], pipeAddrs[2],pipeAddrs[1],pipeAddrs[0]);
	API_DEBUG_MESSAGE(uartTxBuf);

	NRF24_readRegisterN(radio, 0x0A+2, pipeAddrs, 1);
	sprintf(uartTxBuf, "RX_Pipe2 Addrs:\r\n		xx,xx,xx,xx,%02X  \r\n", pipeAddrs[0]);
	API_DEBUG_MESSAGE(uartTxBuf);

	NRF24_readRegisterN(radio, 0x0A+3, pipeAddrs, 1);
	sprintf(uartTxBuf, "RX_Pipe3 Addrs:\r\n		xx,xx,xx,xx,%02X  \r\n", pipeAddrs[0]);
	API_DEBUG_MESSAGE(uartTxBuf);

	NRF24_readRegisterN(radio, 0x0A+4, pipeAddrs, 1);
	sprintf(uartTxBuf, "RX_Pipe4 Addrs:\r\n		xx,xx,xx,xx,%02X  \r\n", pipeAddrs[0]);
	API_DEBUG_MESSAGE(uartTxBuf);

	NRF24_readRegisterN(radio, 0x0A+5, pipeAddrs, 1);
	sprintf(uartTxBuf, "RX_Pipe5 Addrs:\r\n		xx,xx,xx,xx,%02X  \r\n", pipeAddrs[0]);
	API_DEBUG_MESSAGE(uartTxBuf);

	NRF24_readRegisterN(radio, 0x0A+6, pipeAddrs, 5);
	sprintf(uartTxBuf, "TX Addrs:\r\n		%02X,%02X,%02X,%02X,%02X  \r\n", pipeAddrs[4], pipeAddrs[3], pipeAddrs[2],pipeAddrs[1],pipeAddrs[0]);
	API_DEBUG_MESSAGE(uartTxBuf);

	//h) RX PW (Payload Width 0 - 32)
	reg8Val = NRF24_readRegister(radio, 0x11);
	sprintf(uartTxBuf, "RX_PW_P0:\r\n		%d bytes \r\n", reg8Val&0x3F);
	API_DEBUG_MESSAGE(uartTxBuf);

	reg8Val = NRF24_readRegister(radio, 0x11+1);
	sprintf(uartTxBuf, "RX_PW_P1:\r\n		%d bytes \r\n", reg8Val&0x3F);
	API_DEBUG_MESSAGE(uartTxBuf);

	reg8Val = NRF24_readRegister(radio, 0x11+2);
	sprintf(uartTxBuf, "RX_PW_P2:\r\n		%d bytes \r\n", reg8Val&0x3F);
	API_DEBUG_MESSAGE(uartTxBuf);

	reg8Val = NRF24_readRegister(radio, 0x11+3);
	sprintf(uartTxBuf, "RX_PW_P3:\r\n		%d bytes \r\n", reg8Val&0x3F);
	API_DEBUG_MESSAGE(uartTxBuf);

	reg8Val = NRF24_readRegister(radio, 0x11+4);
	sprintf(uartTxBuf, "RX_PW_P4:\r\n		%d bytes \r\n", reg8Val&0x3F);
	API_DEBUG_MESSAGE(uartTxBuf);

	reg8Val = NRF24_readRegister(radio, 0x11+5);
	sprintf(uartTxBuf, "RX_PW_P5:\r\n		%d bytes \r\n", reg8Val&0x3F);
	API_DEBUG_MESSAGE(uartTxBuf);

	//i) Dynamic payload enable for each pipe
	reg8Val = NRF24_readRegister(radio, 0x1c);
	sprintf(uartTxBuf, "DYNPD_pipe:\r\n		P0:	%d\r\n		P1:	%d\r\n		P2:	%d\r\n		P3:	%d\r\n		P4:	%d\r\n		P5:	%d\r\n",
	_BOOL(reg8Val&(1<<0)), _BOOL(reg8Val&(1<<1)), _BOOL(reg8Val&(1<<2)), _BOOL(reg8Val&(1<<3)), _BOOL(reg8Val&(1<<4)), _BOOL(reg8Val&(1<<5)));
	API_DEBUG_MESSAGE(uartTxBuf);

	//j) EN_DPL (is Dynamic payload feature enabled ?)
	reg8Val = NRF24_readRegister(radio, 0x1d);
...
/*void printStatusReg(nrf24_t * const radio)
{
	...
	reg8Val = NRF24_readRegister(radio, 0x07);
...
/*void printFIFOstatus(nrf24_t * const radio)
{
	...
	reg8Val = NRF24_readRegister(radio, 0x17);
